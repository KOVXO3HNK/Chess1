...
которым разработчики chess.js собирают готовый модуль из грамматики. Никаких дополнительных зависимостей подключать не нужно: в репозитории
уже лежит сгенерированный итоговый файл движка.
...
2. Выполните встроенный скрипт контроля целостности:
   ```bash
   node scripts/check-dom.mjs
   ```
   Скрипт проверяет, что все `id`, используемые в `public/app.js`, присутствуют в `public/index.html`, а модуль `public/chess.js` корректно инициализирует
   шахматный движок и позволяет выполнять базовые ходы.

## Известные ограничения

- Игра с реальными соперниками реализована только в режиме «за одним устройством» (приглашение отправляется ссылкой или сообщением).
- Для игры против ИИ используется встроенный минимакс с эвристикой, поэтому глубина ограничена тремя полуходами.
